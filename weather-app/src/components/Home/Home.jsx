...
const Home = () => {
	...
	const fetchWeatherData = async (latitude, longitude, city = null) => {
		try {
			const searchQuery = city ? city : `${latitude},${longitude}`

			const currentWeatherResponse = await fetch(
				`${api.base}current.json?key=${api.key}&q=${searchQuery}`
			)
			...
			const forecastResponse = await fetch(
				`${api.base}forecast.json?key=${api.key}&q=${searchQuery}&days=3`
			)
			...
			if (
				...
				const minTemp = filteredData.reduce(
					(min, data) => Math.min(min, data.temp_f),
					Infinity
				)
				const maxTemp = filteredData.reduce(
					(max, data) => Math.max(max, data.temp_f),
					-Infinity
				)
		...
		>
			<div className="top-container">
				<div className="search-container">
					...
					<div className='temp-two-wrapper'>
					{weatherData && (
						<div className="temp">
									...
									<div className="row-wrapper">
										<div className="container">
											...
											<p>
												...
												{weatherData.current.wind_mph} mp/h
											</p>
										</div>
										<div className="container">
											<p>Feels Like</p>
