...
const Home = () => {
  ...
  const fetchWeatherData = async (latitude, longitude, city = null) => {
    try {
      ...
      const currentWeatherResponse = await fetch(`${api.base}current.json?key=${api.key}&q=${searchQuery}`);
      if (!currentWeatherResponse.ok) {
        throw new Error('City not found');
      }
      const currentWeatherResult = await currentWeatherResponse.json();

      const forecastResponse = await fetch(`${api.base}forecast.json?key=${api.key}&q=${searchQuery}&days=3`);
      if (!forecastResponse.ok) {
        throw new Error('City not found');
      }
      const forecastResult = await forecastResponse.json();
